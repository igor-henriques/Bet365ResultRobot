...
public class PremiershipWorker : BackgroundService, IWorker
{
    ...
    private readonly OddBuilder<PremiershipWorker> oddBuilder;

    public PremiershipWorker(IWebRepository webRepository, ILogger<PremiershipWorker> logger, ElementsXPath elementsXPath,
        ElementsCSS elementsCSS, Settings settings, IOddRepository oddRepository, ITeamRepository teamRepository, IMatchNextRepository matchNextRepository)
    {
        ...
        this.oddBuilder = new OddBuilder<PremiershipWorker>(webRepository, settings, elementsXPath, elementsCSS, logger);
    }
    ...
    public async Task DoWorkAsync()
    {
        if (!webRepository.GetCurrentURL().Equals(elementsXPath.VirtualFutebolURL))
        {
            ...
            await webRepository.ClickOnElement(By.XPath(elementsXPath.PremiershipElements.BrowserTabPath));
        }

        while (!await CheckEventTabAvailable())
            await webRepository.ClickOnElement(By.XPath(elementsXPath.PremiershipElements.BrowserTabPath), false);
    ...
    public async Task ResolveData(Odd odd, string nomeTimeVisitante, string nomeTimeCasa, string eventSchedule)
    {
        if (odd != null)
        {
            ...
            {
                IdCompetition = settings.PremiershipIdCompetition,
