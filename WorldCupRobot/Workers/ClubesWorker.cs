...
internal class ClubesWorker : BackgroundService, IWorker
{
    public static bool IsAlive = false;

    private readonly Random random = new Random();
    private readonly ILogger<ClubesWorker> logger;
    ...
    public ClubesWorker(ILogger<ClubesWorker> logger, IOddRepository oddContext,
    ...
    public Task<bool> CheckEventTabAvailable()
    {
    ...
    public async Task<bool> IsEventStarted(IWebElement _event, int eventIndex)
    {
        try
        {
            ...
            var eventStartedSpan = await webContext.GetElement(By.XPath(string.Format(locators.AoVivoDescriptionClubesXPath, eventIndex + 1)), false);
    ...
    public async Task ScrapAllEvents()
    {
        try
        {
            if (!webContext.GetCurrentURL().Equals(locators.MainURL + locators.ClubesSubURL))
            {
                webContext.Navigate(locators.MainURL + locators.ClubesSubURL);
    ...
    {
        var oddBuilder = new OddBuilder<ClubesWorker>(webContext, locators, logger);
