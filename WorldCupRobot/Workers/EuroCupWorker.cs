...
public class EuroCupWorker : BackgroundService, IWorker
{
    ...
    private readonly OddBuilder<EuroCupWorker> oddBuilder;

    public EuroCupWorker(IWebRepository webRepository, ILogger<EuroCupWorker> logger, ElementsXPath elementsXPath,
        ElementsCSS elementsCSS, Settings settings, IOddRepository oddRepository, ITeamRepository teamRepository, IMatchNextRepository matchNextRepository)
    {
        ...
        this.oddBuilder = new OddBuilder<EuroCupWorker>(webRepository, settings, elementsXPath, elementsCSS, logger);
    }
    ...
    public async Task DoWorkAsync()
    {
        if (!webRepository.GetCurrentURL().Equals(elementsXPath.VirtualFutebolURL))
        {
            ...
            await webRepository.ClickOnElement(By.XPath(elementsXPath.EurocupElements.BrowserTabPath));
        }

        while (!await CheckEventTabAvailable())
            await webRepository.ClickOnElement(By.XPath(elementsXPath.EurocupElements.BrowserTabPath), false);
